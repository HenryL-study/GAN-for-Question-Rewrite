...
def get_gen_ans(trainable_model, data_loader, gen_ans_file, epoch, summary_w):
    generated_answers = []
    data_loader.reset_pointer()
    for _ in range(data_loader.num_test_batch):
        ...
        summary_w.add_summary(att_img, _)
        
        image_file = "image/attention_test" + str(epoch)
        print("save attention image to %s :" % image_file)
        image_summ = tf.Summary()
        image_summ.ParseFromString(att_img)
        for idx in range(BATCH_SIZE):
            with tf.gfile.GFile(image_file + '_idx_' + str(idx + BATCH_SIZE * _) + ".png", mode="w") as img_f:
                img_f.write(image_summ.value[idx].image.encoded_image_string)
    ...
    for _ in range(20):
        ...
        summary_w.add_summary(att_img, _ + 70)
        
        image_file = "image/attention_train" + str(epoch)
        print("save attention image to %s :" % image_file)
        image_summ = tf.Summary()
        image_summ.ParseFromString(att_img)
        for idx in range(BATCH_SIZE):
...
saver.restore(sess, "save/model/model_cat.ckpt")
print ('Done')

summary_w = tf.summary.FileWriter("tensorboard/", sess.graph)

get_gen_ans(seq2seq_model, data_loader, gen_ans_file, 500, summary_w)
summary_w.close()
