...
class Seq2seq_Model(object):
    ...
    def add_decoder_for_inference(self):
        ...
        predicting_helper = CustomGreedyEmbeddingHelper(self.g_embeddings,
                                                        ...
                                                        self.seq_end_token,
                                                        self.cnn_context)
        predicting_decoder = tf.contrib.seq2seq.BasicDecoder(self.decoder_cell,
                                                        predicting_helper,
                                                        self.decoder_cell.zero_state(
                                                            dtype=tf.float32, batch_size=self.batch_size).clone(cell_state = self.encoder_state),
                                                        self.output_layer)
        ...
        predicting_decoder_output, final_context_state, _ = tf.contrib.seq2seq.dynamic_decode(predicting_decoder,
                                                            impute_finished=True,
                                                            maximum_iterations=self.max_ans_length)
        print("predict decoder output: ", predicting_decoder_output)
        return predicting_decoder_output, final_context_state

    def generate(self, sess, x, x_len, response, res_len):
        res_len_max = max(res_len)
        outputs, att_img = sess.run([self.g_samples, self.infer_summary], feed_dict={self.x: x, self.target_sequence_length: x_len, self.response: response, self.target_response_length: res_len, self.max_response_length_per_batch: res_len_max})
        return outputs, att_img
